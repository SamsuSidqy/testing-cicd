...
class LoggingHelpers{
	...
	public static function CreateLogging($title,$description,$type,$code){
        $agent = LoggingHelpers::getBrowserInfo();
		\App\Models\Developer\Logging::create([
            'title' => $title ?? null,
            'deskripsi' => $description,
            'method' => Request::method() ?? 'CLI',
            'ip4' => Request::ip() ?? '127.0.0.1',
            'url' => Request::fullUrl() ?? 'N/A',
            'device' => $agent['device'] ?? 'Unknown',
            'platform' => $agent['os_platform'] ?? 'Unknown',
            'agent' => $agent['user_agent'] ?? 'Unknown',
            'browser' => $agent['browser'] ?? 'Unknown',
            'browser_version' => $agent['browser_version'] ?? 'Unknown',
            'users' => Auth::check() ? Auth::user()->id_users : null,
            'type' => $type,
            'status_code' => $code ?? 500,
            'data' => json_encode(Request::all()) ?? null
        ]);         
	}
